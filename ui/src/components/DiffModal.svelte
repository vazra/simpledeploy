...
<div class="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true">
  ...
  <div class="relative bg-surface-2 border border-border rounded-lg p-6 w-full max-w-3xl mx-4 shadow-xl flex flex-col" style="max-height: 90vh;">
    ...
    <div class="flex-1 overflow-y-auto min-h-0 mb-5">
      <pre class="font-mono text-xs bg-surface-1 border border-border rounded-lg overflow-auto" style="max-height: 60vh;"><code>{#each parts as part}{#if part.added}{#each getLines(part.value) as line}<span class="block bg-success/10 text-success px-2">+ {line}</span>{/each}{:else if part.removed}{#each getLines(part.value) as line}<span class="block bg-danger/10 text-danger px-2">- {line}</span>{/each}{:else}{#each getLines(part.value) as line, i}{#if i < 3 || getLines(part.value).length - i <= 3}<span class="block text-text-secondary px-2">  {line}</span>{:else if i === 3 && getLines(part.value).length > 6}<span class="block text-text-muted px-2 italic">  ... {getLines(part.value).length - 6} unchanged lines ...</span>{/if}{/each}{/if}{/each}</code></pre>
    </div>
