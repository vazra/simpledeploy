...
**Status:** All critical/high/medium issues fixed (see commit history)
...
### 18. [OPEN] No Account Lockout
- **Issue:** No tracking of failed login attempts per username.
- **Fix:** Exponential backoff or temporary lockout after N failures.

### 19. [OPEN] Rate Limiter Ineffective Behind Proxy
- **Location:** `internal/api/auth.go:14-16`
- **Issue:** Uses `r.RemoteAddr` only. Behind proxy, all requests share one IP.
- **Fix:** Check `X-Forwarded-For` when behind trusted proxy.
