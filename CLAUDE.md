...
- **Image mirror.** When `SIMPLEDEPLOY_IMAGE_MIRROR_PREFIX` env var is set (e.g. `ghcr.io/vazra/simpledeploy-mirror/`), the server rewrites docker.io-bound image refs in compose files at deploy/rollback/restore time. Used by E2E (`make e2e-mirror`) to avoid Docker Hub pull rate limits, but also usable for local dev. Mirror is populated by `.github/workflows/mirror-images.yml`, which runs on changes to template JS files or e2e fixtures. See `internal/mirror/`. Community recipe images are mirrored by an analogous workflow in `vazra/simpledeploy-recipes`.
- **Community recipes.** Server fetches a JSON catalog from `recipes_index_url` (default `https://vazra.github.io/simpledeploy-recipes/index.json`) on demand, with a 10-minute TTL cache and stale-on-error fallback. Recipe pulls are logged to `recipe_pulls` for popularity ranking (super_admin can query via `/api/recipes/community/popularity`). UI surfaces a "Browse community recipes" button in the deploy wizard. Catalog source repo: `vazra/simpledeploy-recipes`.
...
- **Realtime events.** In-process pub/sub bus (`internal/events`) fans out notify-only events to a single per-tab WebSocket at `GET /api/events`. Audit recorder publishes after every mutation (category-to-topic mapping in `internal/events/topics.go`); reconciler publishes on app status flips. Frontend store `ui/src/lib/stores/realtime.svelte.js` lets components register `(topic, refetchFn)` pairs and refetch via REST. No payload data on the socket; REST is the source of truth. Topics: `app:<slug>`, `global:apps`, `global:settings`, `global:users`, `global:registries`, `global:alerts`, `global:backups`, `global:docker`, `global:audit`. Subscribe-time authz; super_admin sees all globals, others gated by `user_app_access`.
